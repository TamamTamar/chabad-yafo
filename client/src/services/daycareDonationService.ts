...
export const getAdminDaycareExchangeRate = async (
    currency: DaycareForeignCurrency,
    date?: string
) => {
    const response = await http.get<ApiResponse<DaycareExchangeRate>>(
        `/admin/daycare/donations/exchange-rates/${currency}`,
