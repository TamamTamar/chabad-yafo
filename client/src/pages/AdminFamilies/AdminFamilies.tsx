...
const AdminFamilies = () => {
    ...
    if (loading) {
        return <div className={styles.page}>טוען...</div>;
    }
    ...
    return (
        <main className={styles.page}>
            <section className={styles.header}>
                <h1>משפחות שנרשמו</h1>
                <p>רשימת משפחות שהתעניינו בפעילות, חוגים ומסגרות חינוך של בית חב״ד יפו.</p>
            </section>

            <section className={styles.card}>
                {families.length === 0 ? (
                    <div className={styles.empty}>עדיין אין משפחות רשומות</div>
                ) : (
                            ...
                            <tbody>
                                {families.map((family) => (
                                    <tr key={family._id}>
                                        ...
                                        <td className={styles.phone}>{family.phone}</td>
                                        <td>{family.area}</td>

                                        <td>
                                            <div className={styles.tagList}>
                                                {family.ages.map((age) => (
                                                    <span key={age} className={styles.tag}>
                                                        {age}
                                                    </span>
                                                ))}
                                            </div>
                                        ...
                                        <td>
                                            <span className={family.updates ? styles.yes : styles.no}>
