...
const field = (document: PDFKit.PDFDocument, label: string, value: string) => {
    document.font("AssistantBold").fontSize(10).fillColor("#0b3158");
    rtl(document, label);
    document.moveDown(0.18).font("Assistant").fontSize(10.5).fillColor("#172033");
    const text = value.trim() || "לא צוין";
    const height = Math.max(30, document.heightOfString(prepareMixedRtlText(text), { width: 455, features: ["rtla"] }) + 14);
    const y = document.y;
...
export const createSignedHealthDeclarationPdf = (input: HealthDeclarationPdfInput) =>
    new Promise<Buffer>((resolve, reject) => {
        const document = new PDFDocument({
            size: "A4",
            margins: { top: 116, right: 54, bottom: 54, left: 54 },
            bufferPages: true,
        ...
        document.moveDown(0.25).font("Assistant").fontSize(10).fillColor("#526174");
        ...
        document.moveDown(0.45).font("Assistant").fontSize(10.5).fillColor("#172033");
        ...
        document.font("Assistant").fontSize(8).fillColor("#64748b");
...
export const createBlankHealthDeclarationPdf = (input: BlankHealthDeclarationPdfInput) =>
    new Promise<Buffer>((resolve, reject) => {
        const document = new PDFDocument({ size: "A4", margins: { top: 116, right: 54, bottom: 54, left: 54 }, bufferPages: true, info: { Title: `הצהרת בריאות למילוי - ${input.childName}`, Author: "מעון חב״ד יפו" } });
        const chunks: Buffer[] = [];
        document.on("data", (chunk: Buffer) => chunks.push(chunk));
        document.on("error", reject);
        document.on("end", () => resolve(Buffer.concat(chunks)));
        ...
        rtl(document, "הצהרה ואישור");
        document.moveDown(0.35).font("Assistant").fontSize(10).fillColor("#172033");
