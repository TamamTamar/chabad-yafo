...
const encryptedValueSchema = new Schema({
    ...
    ciphertext: { type: String, required: true, maxlength: 30000, immutable: true },
}, { _id: false });
...
export const daycarePickupAuthorizationSchema = new Schema<IDaycarePickupAuthorization>({
    ...
    signingMethod: { type: String, enum: ["online", "uploadedFile"], required: true, immutable: true },
...
daycarePickupAuthorizationSchema.index({ onboardingId: 1, revision: 1 }, { unique: true });
